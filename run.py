...
if args.variant == 'vanilla':
    ...
    synthesizer_mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
    n_layer=4, n_head=8, n_embd=256, attention = 2)
    model = model.GPT(synthesizer_mconf)
...
if args.function == 'pretrain':
    ...
    pretrain_tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3,
                      lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                      num_workers=4)
    trainer = trainer.Trainer(model, pretrain_dataset, None, pretrain_tconf)
    trainer.train()
